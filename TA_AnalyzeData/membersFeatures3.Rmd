...
Нормализация гиперболическим тангенсом дает нам значения в диапазоне (-1,1), сохраняя 0, т.е. если изначально значение было равно 0, то после нормализации оно также будет равно 0. Также "растягиваем" шкалу нормализации таким образом, чтобы значения **VIPBalanceCount** с диапазоне [0;10] растягивались приблизительно в диапазон [0,0.9], т.е. были хорошо отличимы между собой, а значения более 10 "схлопывались"в районе +1.
...
dfCPCount <-     dfMembersCP %>% select (uid, placeID, reviewID) %>%
    ...
    mutate (VIPPlacesCount = ifelse(is.na(VIPPlacesCount), 0, VIPPlacesCount),
            ...
            VIPBalanceCount = tanh((VIPPlacesCount-nonVIPPlacesCount)/6)) %>%
...
dfTmp2 <-    dfTmp %>% select (uid, category) %>%    
    ...
    summarize(categoryCount=n()) %>% 
    ungroup() %>% 
    mutate (categoryCountNorm = tanh(categoryCount/18)) %>% 
...
dfTmp4 <-    dfTmp %>% select (uid, userRating) %>%    
    ...
    summarize(userRatingCount=n()) %>% 
    ungroup() %>% 
    mutate (userRatingNorm = tanh(userRatingCount/18)) %>% 
