---
title: "Подготовка параметров пользователя Трипадвизора. Итерация 2"
...
На основе данных об отзывах пользователя о месте (MembersCityPage) посчитаем следующие параметры:
 ...
 - **VIPPlacesCount** - количество посещенных VIP мест.
 - **nonVIPPlacesCount** - количество посещенных не-VIP мест.
 - **VIPBalanceCount** - разница между количеством посещенных VIP и не-VIP мест.
 
```{r addPlacesFeatures, echo=FALSE, warning=FALSE, message = FALSE}
#View(dfMembersCP) #MembersCityPage
...
- **userRating*Percent** - доля отзывов о местах c пользовательским рейтингм 1-5 от общего числа отзывов.
...
#View(dfPRCount)
categoryFeatures <- colnames(dfPRCount)[-1]

# аналогичный расчет, но не для пользовательского рейтинга
dfTmp2 <-    dfTmp %>% select (uid) %>%    
    count(uid) %>% 
    inner_join(dfTmp, by=c("uid"="uid")) %>%
...
#View(dfTmp2)
userRatingFeatures <- colnames(dfTmp2)[-1]
...
- **VIPPPYCount** - среднее количество посещенных VIP мест в год.
- **nonVIPPPYCount** - среднее количество посещенных не-VIP мест в год.
...
dfMOMos <- dfMOMos %>% 
    mutate (VIPPPYCount = VIPPlacesCount/(maxYear-registrationYear),
...
# ### Нормализуем линейно в диапазон [-x,+1]: VIPBalanceCount, VIPBPYCount
# #reviews*Count уже номализованы
# normalizeList3 <- c("VIPBalanceCount", "VIPBPYCount") 
# # preProcNumeric3 <- preProcess (dfMOMos[normalizeList3], method=c("scale") )
# # dfNormalized3 <- data.frame(uid = dfMOMos$uid, predict(preProcNumeric3,dfMOMos[normalizeList3]),
# #                            stringsAsFactors = FALSE)
# # #dfNormalized3[normalizeList3] <- dfNormalized3[normalizeList3]*2-1
# 
# dfNormalized3 <- dfMOMos[c("uid", normalizeList3)]
# dfNormalized3$VIPBalanceCount <- dfNormalized3$VIPBalanceCount/max(dfNormalized3$VIPBalanceCount)
# dfNormalized3$VIPBPYCount <- dfNormalized3$VIPBPYCount/max(dfNormalized3$VIPBPYCount)
# #View(dfNormalized3)
# #View(dfNormalized3[dfNormalized3$VIPBalanceCount<0,])
# #View(dfMOMos[dfMOMos$VIPBalanceCount<0,])
...
#View (dfMOMosNormalized3)

saveRDS (dfMOMosNormalized3, "../data/MaldivesMOMosNormalized_v3.rds") #без  посещенных отелей
#saveRDS (dfMOMosNormalized3, "../data/MaldivesMOMosNormalized_v3b.rds") #3b. с посещенными отелями
```
...
dfTrain <- dfMOMosNormalized3[!is.na(dfMOMosNormalized3$class2),]
#View (dfTrain)
#countFeatures[1:5] Отсутствуют, т.к. reviews*Count заменены на reviews*Percent
featurePlot (x = dfTrain[ ,c(c(countFeatures[c(6,7,16,23)]),"travelStatDistance")],
             y = dfTrain$class2,
             plot="pairs", auto.key=list(columns=2))
featurePlot (x = dfTrain[ ,countFeatures[8:15]],
             y = dfTrain$class2,
             plot="pairs", auto.key=list(columns=2))
featurePlot (x = dfTrain[ ,countFeatures[16:22]],
             y = dfTrain$class2,
             plot="pairs", auto.key=list(columns=2))
...
featurePlot (x = dfTrain[ ,userRatingFeatures],
             y = dfTrain$class2,
             plot="pairs", auto.key=list(columns=2))
featurePlot (x = dfTrain[ ,c("category50", "badgeHotelReviewsPercent", "sex", "ageGroup",
